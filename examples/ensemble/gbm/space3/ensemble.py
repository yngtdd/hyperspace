...
def ensemble(clfs, X_train, y_train, X_test, y_test, nfolds=5):
    ...
    * `y_proba` [ndarray]
    ...
    for iteration, clf in enumerate(clfs):
        ...
        print(f'Hyperparameters: max_depth: {clf.max_depth}, learning_rate: {clf.learning_rate:.4f}, min_impurity_decrease: {clf.min_impurity_decrease}')
        ...
        dataset_blend_test_iter = np.zeros((X_test.shape[0], n_models))
...
def main():
    ...
    for result in results:
        ...
        lr = hyperparams[1]
        min_impurity_decrease = hyperparams[2]
